...
/*
document.getElementById('omikuji-button').addEventListener('click', async () => {
    const spinner = document.getElementById('omikuji-spinner');
    const omikujiText = document.getElementById('omikuji-text');
    spinner.style.display = 'flex'; // スピナーを表示

    const omikujiResults = ["大吉", "中吉", "小吉", "凶"];
    let interval;
    ...
    try {
        ...
        // モバイル環境：aLiceを起動して公開鍵を取得
        if (window.innerWidth <= 768) {
            const callbackUrl = `${window.location.origin}/alice-callback`;
            const encodedCallbackUrl = Array.from(new TextEncoder().encode(callbackUrl))
                .map(byte => byte.toString(16).padStart(2, '0'))
                .join('');
            const aliceUrl = `alice://sign?type=request_pubkey&callback=${encodedCallbackUrl}`;
            window.location.href = aliceUrl;
        ...
        if (window.SSS && window.SSS.activePublicKey) {
            ...
            const publicAccount = facade.createPublicAccount(new sdkCore.PublicKey(publicKey));
            accountAddress = publicAccount.address;
        } else {
            Swal.fire({
                title: "エラー",
                text: "SSS Extensionが有効ではありません。",
                icon: "error",
                confirmButtonText: "OK",
            });
            spinner.style.display = 'none'; // スピナーを非表示
        ...
        // 過去24時間以内のおみくじ履歴をチェック
        const hasRecentOmikuji = await checkRecentOmikuji(accountAddress.toString());

        if (hasRecentOmikuji) {
            Swal.fire({
                title: "おみくじ制限",
                text: "過去24時間以内におみくじを引いています。時間をおいて再度お試しください。",
                icon: "info",
                confirmButtonText: "OK",
            });
            spinner.style.display = 'none'; // スピナーを非表示
        ...
        // アニメーション開始（結果を高速で切り替える）
        interval = setInterval(() => {
            const randomIndex = Math.floor(Math.random() * omikujiResults.length);
            omikujiText.textContent = omikujiResults[randomIndex];
        }, 100); // 0.1秒ごとに結果を切り替え

        // 3秒後に最終結果を決定
        setTimeout(async () => {
            clearInterval(interval); // アニメーションを停止
            const finalResult = omikujiResults[Math.floor(Math.random() * omikujiResults.length)];
            omikujiText.textContent = finalResult; // 最終結果を表示
            ...
            setTimeout(async () => {
                ...
                // 結果をSwal.fireで表示
                Swal.fire({
                    title: finalResult === "大吉"
                        ? `<h2 class="custom-swal-title-dakichi">🎉 おみくじ結果: 大吉 🎉</h2>`
                        : `<h2 class="custom-swal-title-default">おみくじ結果: ${finalResult}</h2>`,
                    html: finalResult === "大吉"
                        ? `<div class="custom-swal-content-dakichi">🌟 おめでとうございます！ 🌟 <br>50XYMが送信されます！</div>`
                        : `<div class="custom-swal-content-default">また挑戦してみてください！</div>`,
                    iconHtml: finalResult === "大吉"
                        ? `<div class="custom-icon-wrapper custom-icon-wrapper-dakichi">🎊</div>`
                        : `<div class="custom-icon-wrapper custom-icon-wrapper-default">🔮</div>`,
                    showConfirmButton: true,
                    confirmButtonText: "OK",
                    confirmButtonColor: finalResult === "大吉" ? "#ff4500" : "#4caf50",
                    customClass: {
                        popup: finalResult === "大吉" ? "custom-swal-popup-dakichi" : "custom-swal-popup-default",
                    },
                    background: finalResult === "大吉"
                        ? "linear-gradient(135deg, #ffe4e1, #f8a5c2)"
                        : "#ffffff"
                });

                // トランザクションをバックエンドに送信
                const rewardAmount = finalResult === "大吉" ? 50 : 0; // 大吉の場合は50XYM、その他は0XYM
                await Omikuji_reward(accountAddress, rewardAmount, finalResult);
            }, 2000);
        }, 3000); // 3秒間アニメーションを回す
    } catch (error) {
        console.error("おみくじ処理中にエラーが発生しました:", error);

        // エラー時にSwal.fireで通知
        Swal.fire({
            title: "エラー",
            text: "おみくじの処理に失敗しました。再度お試しください。",
            icon: "error",
            confirmButtonText: "OK"
        });
...
async function Omikuji_reward(accountAddress, amount, result) {
    try {
        const response = await fetch('https://vmi1152053.contaboserver.net:5000/Omikuji_reward', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ recipientAddress: accountAddress.toString(), amount, result })
        });

        if (!response.ok) {
            console.error('バックエンドで報酬送信失敗');
            Swal.fire({
                title: "エラー",
                text: "報酬の送信に失敗しました。",
                icon: "error",
        ...
        if (responseData.success) {
            ...
            console.error('バックエンドで報酬処理失敗:', responseData.message || "不明なエラー");
            Swal.fire({
                title: "エラー",
                text: responseData.message || "報酬の送信に失敗しました。",
                icon: "error",
                confirmButtonText: "OK"
            });
        }
    } catch (error) {
        console.error('Omikuji rewardエラー:', error);
        Swal.fire({
            title: "エラー",
            text: "報酬の送信中に問題が発生しました。",
            icon: "error",
            confirmButtonText: "OK"
        });
    }
...
/*
async function checkRecentOmikuji(accountAddress) {
    const NODE = 'https://symbol-mikun.net:3001'; // 使用するノード
    const BLOCKS_24H = 2880; // 過去24時間のブロック数
    const latestBlock = await fetch(`${NODE}/chain/info`)
        .then((res) => res.json())
        .then((data) => data.height);

    const startHeight = latestBlock - BLOCKS_24H; // 過去24時間の最初のブロック
    const params = {
        address: accountAddress,
        startHeight: startHeight,
        pageSize: 100, // 1回のリクエストで取得するトランザクション数
        embedded: true,
    };

    let pageNumber = 1; // ページ番号を初期化
    let hasOmikujiResult = false; // おみくじ履歴が見つかったかどうか

    try {
        while (true) {
            // パラメータに現在のページ番号を追加
            const query = new URLSearchParams({ ...params, pageNumber });
            const response = await fetch(`${NODE}/transactions/confirmed?${query}`);
            const result = await response.json();

            // トランザクションが存在しない場合、ループを終了
            if (!result.data || result.data.length === 0) break;

            // 各トランザクションを確認
            for (const tx of result.data) {
                if (tx.transaction.message && tx.transaction.message.includes("おみくじ結果")) {
                    hasOmikujiResult = true; // 履歴が見つかった
                    break;
                }
            }

            // 履歴が見つかったらループを終了
            if (hasOmikujiResult) break;

            // 次のページに進む
            pageNumber++;
        }

        return hasOmikujiResult;
    } catch (error) {
        console.error("おみくじ履歴チェック中にエラー:", error);
        return false; // エラーが発生した場合は履歴なしとする
    }
}
*/
...
async function checkRecentOmikuji(accountAddress) {
    ...
    const startHeight = latestBlock - BLOCKS_24H;
    const params = {
        address: accountAddress,
        startHeight: startHeight,
        pageSize: 100,
    ...
    try {
        const promises = [];
        for (let pageNumber = 1; pageNumber <= 10; pageNumber++) { // 最大10ページを並列に取得
            const query = new URLSearchParams({ ...params, pageNumber });
            promises.push(fetch(`${NODE}/transactions/confirmed?${query}`).then(res => res.json()));
        }
        ...
        for (const result of results) {
            if (result.data) {
                for (const tx of result.data) {
                    if (tx.transaction.message && tx.transaction.message.includes("おみくじ結果")) {
                        return true; // 履歴が見つかった
                    }
                }
            }
        }

        return false; // 履歴が見つからない
    } catch (error) {
        console.error("おみくじ履歴チェック中にエラー:", error);
        return false;
    }
